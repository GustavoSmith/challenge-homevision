...
test.describe("Property detail page", () => {
  test("shows loading skeleton then renders property details on success", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: fakeHouse.address }),
    ).toBeVisible();
  ...
  test("shows error state with retry button when API fails", async ({
    ...
    const alert = page
      .getByRole("alert")
      .filter({ hasText: "couldn't load this property" });
